...
u0 = double(imread('lena_avec_bruit.bmp'));
...
while convergence > critere_arret_convergence
	...
	u_kp1(:,1) = pas_variation_totale(u0(:,1),u_k(:,1),lambda,Dx,Dy,epsilon);
    u_kp1(:,2) = pas_variation_totale(u0(:,2),u_k(:,2),lambda,Dx,Dy,epsilon);
    u_kp1(:,3) = pas_variation_totale(u0(:,3),u_k(:,3),lambda,Dx,Dy,epsilon);
	...
	subplot(1,2,2)
		imagesc(max(0,min(1,reshape(u_k,[nb_lignes nb_colonnes nb_canaux]))),[0 1])
		colormap gray
		axis image off
